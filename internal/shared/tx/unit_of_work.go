...
import (
	...
	"github.com/samurenkoroma/agro-platform/internal/domain/shared/repository"
...
type UnitOfWork interface {
	...
	Tx() *sql.Tx
	RegisterAggregate(agg aggregate.AggregateRoot)
	Commit() error
	Rollback() error
}
...
func (uow *unitOfWork) RegisterAggregate(agg aggregate.AggregateRoot) {
	uow.mu.Lock()
	defer uow.mu.Unlock()
